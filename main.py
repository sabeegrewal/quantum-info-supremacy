...
for seed in range(start_seed, start_seed + n_jobs, n_stitches):
    ...
    for i, s in enumerate(batch):
        ...
        np.random.seed(s)
        opt = optimize(target_states[i], depth, noisy=noisy)
        opt_params = opt.x
        opt_params_list.append(opt_params)

        output_states.append(output_state(n, opt_params))
        noiseless_fidelity = -loss(opt_params, target_states[i])
        noisy_fidelity = fidelity_from_noise(n, opt_params)
        noisy_fidelities.append(noisy_fidelity)
        print(f"Noiseless fidelity: {noiseless_fidelity}")
        print(f"Estimated fidelity due to noise: {noisy_fidelity}")
        print(f"Estimated overall fidelity: {noisy_fidelity * noiseless_fidelity}")
